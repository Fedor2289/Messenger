...
def _add_column_if_missing(conn, table: str, column: str, col_type: str):
    """Добавляет колонку если её нет. Каждый ALTER в savepoint чтобы не ломать транзакцию."""
    try:
        conn.execute(text(f"SAVEPOINT sp_{table}_{column}"))
        conn.execute(text(f"ALTER TABLE {table} ADD COLUMN {column} {col_type}"))
        conn.execute(text(f"RELEASE SAVEPOINT sp_{table}_{column}"))
        logger.info(f"Migration: added {table}.{column}")
    except Exception:
        try:
            conn.execute(text(f"ROLLBACK TO SAVEPOINT sp_{table}_{column}"))
        except Exception:
            pass  # SQLite не поддерживает savepoints — игнорируем


def _create_reactions_table_if_missing(conn):
    """Создаёт таблицу реакций если её нет."""
    try:
        conn.rollback()  # сбрасываем возможный aborted state
    except Exception:
        pass
    try:
        conn.execute(text("""
